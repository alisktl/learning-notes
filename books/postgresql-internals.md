...
- Часть 0.
  - [Глава 1. Введение](#глава-1-введение)
    ...
    2. [Процессы и память](#процессы-и-память)
    3. [Клиенты и клиент - серверный протокол](#клиенты-и-клиент-серверный-протокол)
- Часть I. Изоляция и многоверсионность
  - Глава 2. Изоляция
    1. [Согласованность](#клиенты-и-клиент-серверный-протокол)
    2. [Уровни изоляции и аномалии в стандарте SQL](#уровни-изоляции-и-аномалии-в-стандарте-sql)
    3. [Уровни изоляции в PostgreSQL](#уровни-изоляции-в-postgresql)
...
До сих пор я утверждал, что видимость данных может поменяться только между операторами, но так ли это очевидно? А если запрос выполняется долго, может ли он увидеть часть данных в одном состоянии, а часть – уже в другом?

Проверим, первая строка будет прочитана сразу, а вторая – через две секунды:
```
SELECT amount, pg_sleep(10) -- 10 seconds
...
```
```
SELECT get_amount(id), pg_sleep(10)
FROM accounts WHERE client = 'bob';
```
...
Такой эффект возможен только на уровне изоляции Read Committed и только с категорией изменчивости VOLATILE. Беда в том, что по умолчанию используется именно этот уровень изоляции и именно эта категория изменчивости, так что остается признять – грабли лежат очень удачно.
...
Выполнение оператора UPDATE состоит из двух частей. Сначала фактически выполняется оператор SELECT, который отбирает для обновления строки, соответствующие условию. Поскольку изменения первой транзакции не зафиксировано, вторая транзакция не может его видеть, и оно никак не влияет на выбор строк для начисления процентов. Таким образом, счета Боба попадают под условие, и после выполнения обновления его баланс должен увеличиться на 10 ₽.
...
После снятия блокировки оператор UPDATE *перечитывает* строку, которую пытается обновить (но только ее одну!). В результате получается, что Бобу начислено 9 ₽, исходя из суммы 900 ₽. Но если бы у Боба было 900 ₽, его счета вообще не должны были попасть в выборку.
...
Впрочем, хитрость с перечитыванием заблокированной строки не спасет от потери изменений, если обновление происходит не в одном операторе SQL.
...
```
UPDATE accounts SET amount = 800 + 100 WHERE id = 1
RETURNING amount;
```
output:
```
...
К сожалению, Алиса недосчиталась 100 ₽. СУБД ничего не знает о том, что запомненное значение 800 ₽ как-то связано с *accounts.amount*, и допускает аномалию потерянного изменения. На уровне ихоляции Read Committed такой код некорректен.

#### Repeatable Read
- **Отсутствие неповторяющегося и фантомного чтения.** Само название уровня изоляции [Repeatable Read](postgrespro.ru/docs/postgresql/16/transaction-iso#XACT-REPEATABLE-READ) говорит о повторяемости чтения. Проверим это, а заодно убедимся и в отсутствии фантомов. Для этого в первой транзакции вернем счета Боба в прежнее состояние и создадим новый счет для Чарли:
...
COMMIT;
```
Вторая повторно выполняет тот же самый запрос:
```
SELECT * FROM accounts ORDER BY id;
```
output:
...
- **Ошибка сериализации вместо потерянных изменений.**
